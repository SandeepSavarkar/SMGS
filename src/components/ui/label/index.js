...
class Label extends React.Component {
  ...
  render() {
    ...
    if (this.props.xxxxlarge) stylesArray.push({fontSize: hp(5)});
    else if (this.props.xxxlarge) stylesArray.push({fontSize: hp(3.4)});
    else if (this.props.xxlarge) stylesArray.push({fontSize: hp(2.8)});
    else if (this.props.xlarge) stylesArray.push({fontSize: hp(2.5)});
    else if (this.props.large) stylesArray.push({fontSize: hp(2.1)});
    else if (this.props.normal) stylesArray.push({fontSize: hp(1.9)});
    else if (this.props.small) stylesArray.push({fontSize: hp(1.7)});
    else if (this.props.xsmall) stylesArray.push({fontSize: hp(1.5)});
    else if (this.props.xxsmall) stylesArray.push({fontSize: hp(1.2)});
    else stylesArray.push({fontSize: hp(1.9)});

    if (this.props.sofia_bold)
      stylesArray.push({
        fontFamily:
          Platform.OS == 'ios' ? 'SofiaPro-Bold' : 'Sofia-Pro-Bold-Az',
      });
    else if (this.props.sofia_medium)
      stylesArray.push({
        fontFamily:
          Platform.OS == 'ios' ? 'SofiaPro-Medium' : 'Sofia-Pro-Medium-Az',
      });
    else
      stylesArray.push({
    ...
    return this.props.icon ? (
      <View style={{flexDirection: 'row'}}>
        <Icon name="calendar" size={20} color={Color.PRIMARY_DARK} />
        <Text
